    ...
    def migrate(self, run_id: str) -> bool:
        ...
        try:
            ...
            dump_start_cmd = (
                ...
                f"sudo bash -lc '"
                "set -e; "
                "rm -f /tmp/CRIU-counter/dump.pid /tmp/CRIU-counter/dump.stdout; "
                "nohup criu dump -t "
                f"{pid}"
            ...
            if rc != 0:
                ...
                if rc_src == 0 and counter_b64.strip():
                    self.dest.exec(
            ...
            start_lazy_cmd = (
                ...
                "set -e; "
                "cd /tmp/CRIU-counter; "
                "rm -f /tmp/CRIU-counter/lazy-pages.pid /tmp/CRIU-counter/lazy-pages.stdout; "
                "nohup criu lazy-pages -D /tmp/CRIU-counter "
                ...
                "sleep 0.1; "
                "cat /tmp/CRIU-counter/lazy-pages.pid"
                "'"
            )
            ...
            if expected_after != "unknown" and self.dest.file_exists(out_path):
                ...
                if (
                    ...
                    self.log("WARNING: Could not validate counter continuity")
