...
if [ $# -lt 2 ]; then
  cat <<'EOF'
Usage:
  bash Container/scripts/setup/apply_tc_netem.sh <node> <interface> [--delay 80ms] [--loss 1%] [--rate 20mbit]
  bash Container/scripts/setup/apply_tc_netem.sh <node> <interface> --clear
EOF
  exit 1
fi
