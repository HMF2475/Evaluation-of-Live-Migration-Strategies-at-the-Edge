...
def validate_migration(source_checkpoint: str, target_checkpoint: str = None) -> dict:
    ...
    if source_meta:
        ...
        print(f"  ✓ Source checkpoint accessible")
        print(f"  - Image files: {source_meta.get('image_count', 0)}")
        print(f"  - Total size: {source_meta.get('total_size', 0)} bytes ({source_meta.get('total_size', 0) / 1024:.1f} KB)")
        ...
        if has_required:
            print(f"  ✓ Has required components (core + memory)")
        else:
        ...
        print(f"  ✗ Source checkpoint not accessible")
    ...
    if target_checkpoint and Path(target_checkpoint).exists():
        ...
        if target_meta:
            ...
            print(f"  ✓ Target checkpoint accessible")
            print(f"  - Image files: {target_meta.get('image_count', 0)}")
            print(f"  - Total size: {target_meta.get('total_size', 0)} bytes ({target_meta.get('total_size', 0) / 1024:.1f} KB)")
            ...
            if source_meta and target_meta:
                ...
                if source_count == target_count:
                    ...
                    print(f"  ✗ Image count differs: source={source_count}, target={target_count}")
        else:
            report["validation_results"]["target"] = {
                "accessible": False,
                "error": "Failed to read checkpoint"
            }
            print(f"  ✗ Target checkpoint not accessible")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python3 validate_migration.py <source_checkpoint> [target_checkpoint]")
        print("")
        print("Example:")
    ...
    return 0 if report["validation_results"].get("comparison", {}).get("status") == "PASS" else 1
