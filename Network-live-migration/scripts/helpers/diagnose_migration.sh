...
echo "════════════════════════════════════════════════════════════"
echo "DIAGNOSTIC: CRIU TCP Client Migration"
echo "════════════════════════════════════════════════════════════"
...
PID=$(multipass exec "$SOURCE" -- bash -lc "cat /home/ubuntu/tcp_client.pid 2>/dev/null || cat /home/ubuntu/client.pid 2>/dev/null" | tr -d '\r') || PID=""
if [[ -z "$PID" ]]; then
...
if multipass exec "$SOURCE" -- bash -lc "sudo test -d /tmp/CRIU-tcp-client"; then
  echo "  ✓ /tmp/CRIU-tcp-client exists"
  multipass exec "$SOURCE" -- bash -lc "sudo ls -lh /tmp/CRIU-tcp-client/ | head -15"
else
  echo "  ✗ /tmp/CRIU-tcp-client missing"
fi

# Check 5: Destination output file
echo ""
echo "✓ Check 5: Destination Client Output"
OUT_FILE="/home/ubuntu/tcp_client.out"
...
if multipass exec "$DEST" -- bash -lc "sudo test -f /tmp/CRIU-tcp-client/restored.pid"; then
  RESTORED_PID=$(multipass exec "$DEST" -- bash -lc "sudo cat /tmp/CRIU-tcp-client/restored.pid" | tr -d '\r')
...
if multipass exec "$DEST" -- bash -lc "sudo test -f /tmp/CRIU-tcp-client/restore.log"; then
  multipass exec "$DEST" -- bash -lc "sudo tail -n 30 /tmp/CRIU-tcp-client/restore.log"
else
