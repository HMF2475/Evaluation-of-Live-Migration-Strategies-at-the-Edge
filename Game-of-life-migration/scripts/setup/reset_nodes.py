...
def reset_nodes(source: str, dest: str) -> bool:
    ...
    # Step 1: Kill ALL processes on source (any app.pid or gol.pid)
    log("Step 1: Killing any existing processes on source...")

    # Kill by PID files (generic app.pid and gol.pid)
    for pidfile in ["/home/ubuntu/app.pid", "/home/ubuntu/gol.pid"]:
    ...
    exec_cmd(
        source,
        "ps aux | grep -E 'gol|app\\.py|http\\.server' | grep -v grep | awk '{print $2}' | while read pid; do [ -n \"$pid\" ] && kill -9 \"$pid\" 2>/dev/null || true; done",
    )
    ...
    exec_cmd(source, "rm -f /home/ubuntu/gol.pid /home/ubuntu/gol.out")
    exec_cmd(source, "rm -f /home/ubuntu/*gol* /home/ubuntu/CRIU* 2>/dev/null || true")
    exec_cmd(source, "sudo rm -rf /tmp/CRIU-gol* /tmp/criu* 2>/dev/null || true")
    ...
    log("  Killing all remaining gol processes on destination...")
    exec_cmd(dest, "pkill -9 gol || true")
    exec_cmd(dest, "pkill -f 'native-gol' || true")
    exec_cmd(dest, "pkill -f 'gol\\.sh' || true")
    exec_cmd(
        dest,
        "sudo pkill -9 -f '^criu (dump|restore|page-server|lazy-pages)' 2>/dev/null || true",
    )
    ...
    exec_cmd(dest, "rm -f /home/ubuntu/gol.pid /home/ubuntu/gol.out")
    exec_cmd(dest, "rm -f /home/ubuntu/*gol* /home/ubuntu/*gol.* 2>/dev/null || true")
    exec_cmd(dest, "rm -f /home/ubuntu/CRIU* 2>/dev/null || true")
    exec_cmd(dest, "sudo rm -rf /tmp/CRIU-gol* /tmp/criu* 2>/dev/null || true")
