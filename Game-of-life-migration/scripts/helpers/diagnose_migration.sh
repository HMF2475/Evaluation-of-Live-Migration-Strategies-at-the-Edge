...
PID=$(multipass exec "$SOURCE" -- bash -lc "cat /home/ubuntu/gol.pid 2>/dev/null || cat /home/ubuntu/app.pid 2>/dev/null" | tr -d '\r') || PID=""
if [[ -z "$PID" ]]; then
...
if multipass exec "$SOURCE" -- bash -lc "sudo test -d /tmp/CRIU-gol"; then
  echo "  ✓ /tmp/CRIU-gol exists"
  multipass exec "$SOURCE" -- bash -lc "sudo ls -lh /tmp/CRIU-gol/ | head -15"
else
  echo "  ✗ /tmp/CRIU-gol missing"
fi

# Check 5: Destination output file
echo ""
echo "✓ Check 5: Destination Game of Life Output"
OUT_FILE="/home/ubuntu/gol.out"
...
if multipass exec "$DEST" -- bash -lc "sudo test -f /tmp/CRIU-gol/restored.pid"; then
  RESTORED_PID=$(multipass exec "$DEST" -- bash -lc "sudo cat /tmp/CRIU-gol/restored.pid" | tr -d '\r')
...
if multipass exec "$DEST" -- bash -lc "sudo test -f /tmp/CRIU-gol/restore.log"; then
  multipass exec "$DEST" -- bash -lc "sudo tail -n 30 /tmp/CRIU-gol/restore.log"
else
