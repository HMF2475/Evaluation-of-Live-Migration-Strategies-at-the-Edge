...
def run_strategy(
    *,
    ...
    reset_script = (
        root / "Game-of-life-migration" / "scripts" / "setup" / "reset_nodes.py"
    )
    benchmark = (
        root
    ...
    metrics_dir = root / "Game-of-life-migration" / "metrics" / "node_exporter"
    node_csv = root / "Game-of-life-migration" / "metrics" / "node_exporter_metrics.csv"
    workloads_dir = root / "Game-of-life-migration" / "scripts" / "workloads"

    start_cmd = ["bash", str(workloads_dir / "start_gol_c.sh"), source]
...
def main() -> int:
    ...
    def auto_base_run_id() -> str:
        ...
        # Example:
        #   31-03-2026-gol-cold-precopy-postcopy-h30-d30-i5-netno-snap-235124
        ...
        return (
            f"{date_str}-gol-{strat_part}-h{args.host_runs}-d{args.direct_runs}-"
            f"{it_part}{relay_part}-{snap_part}-{plots_part}-{time_str}"
        )

    base_run_id = args.base_run_id or auto_base_run_id()

    log_dir = root / "Game-of-life-migration" / "metrics" / "run_logs"
    ...
    csv_path = (
        Path(args.csv)
        if args.csv
        else (root / "Game-of-life-migration" / "metrics" / "migration_metrics.csv")
    )
    ...
    with log_path.open("a", encoding="utf-8") as lf:
        ...
        lf.write(
            f"strategies={','.join(strategies)} workload=gol "
    ...
    if not args.no_plots:
        ...
        subprocess.run(
            [
                ...
                str(out_dir),
                "--node-metrics-dir",
                str(root / "Game-of-life-migration" / "metrics" / "node_exporter"),
                "--profile-name",
                args.profile_name,
            ],
            check=False,
